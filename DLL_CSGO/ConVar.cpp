...
// Most of these are copied from sdk.
...
{
    if ( !Cvar.pAddress || s_bRegistered )
        return;
...
ConCommand::ConCommand( const char *pName , FnCommandCallbackV1_t callback , const char *pHelpString /*= 0*/ , int flags /*= 0*/ , FnCommandCompletionCallback completionFunc /*= 0*/ )
{
    ...
    m_fnCompletionCallback = completionFunc ? completionFunc : 0;
...
{
    char* pszOldValue = ( char* ) Alloc( m_Value.m_StringLength );
...
void ConVar::InternalSetFloatValue( float fNewValue )
{
    ...
    m_Value.m_fValue = fNewValue;
    m_Value.m_nValue = ( uint32_t ) fNewValue;
