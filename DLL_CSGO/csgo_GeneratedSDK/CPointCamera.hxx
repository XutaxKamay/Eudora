...
#ifndef SDK_CPointCamera
#define SDK_CPointCamera
#pragma once
 ...
 public:
   ...
   class DT_PointCamera
   {
    public:
      ...
      template<typename T = unsigned long> __forceinline static T FOV() { return ( T ) 0x9D8; }
      template<typename T = unsigned long> __forceinline static T Resolution() { return ( T ) 0x9DC; }
      template<typename T = unsigned long> __forceinline static T bFogEnable() { return ( T ) 0x9E0; }
      template<typename T = unsigned long> __forceinline static T FogColor() { return ( T ) 0x9E1; }
      template<typename T = unsigned long> __forceinline static T flFogStart() { return ( T ) 0x9E8; }
      template<typename T = unsigned long> __forceinline static T flFogEnd() { return ( T ) 0x9EC; }
      template<typename T = unsigned long> __forceinline static T flFogMaxDensity() { return ( T ) 0x9F0; }
      template<typename T = unsigned long> __forceinline static T bActive() { return ( T ) 0x9F4; }
      template<typename T = unsigned long> __forceinline static T bUseScreenAspectRatio() { return ( T ) 0x9F5; }
   };
};
