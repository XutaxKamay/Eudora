...
#ifndef SDK_CPropVehicleChoreoGeneric
#define SDK_CPropVehicleChoreoGeneric
#pragma once
       ...
       public:
         ...
         class DT_BreakableProp
         {
          public:
            ...
            template<typename T = unsigned long> __forceinline static T qPreferredPlayerCarryAngles() { return ( T ) 0x2974; }
            template<typename T = unsigned long> __forceinline static T bClientPhysics() { return ( T ) 0x2980; }
         };

         template<typename T = unsigned long> __forceinline static T bUseHitboxesForRenderBox() { return ( T ) 0x2994; }
         template<typename T = unsigned long> __forceinline static T flGlowMaxDist() { return ( T ) 0x29B4; }
         template<typename T = unsigned long> __forceinline static T bShouldGlow() { return ( T ) 0x29B8; }
         template<typename T = unsigned long> __forceinline static T clrGlow() { return ( T ) 0x29B9; }
         template<typename T = unsigned long> __forceinline static T nGlowStyle() { return ( T ) 0x29C0; }
      };

      template<typename T = unsigned long> __forceinline static T hPlayer() { return ( T ) 0x29E4; }
      template<typename T = unsigned long> __forceinline static T bEnterAnimOn() { return ( T ) 0x29EC; }
      template<typename T = unsigned long> __forceinline static T bExitAnimOn() { return ( T ) 0x29ED; }
      template<typename T = unsigned long> __forceinline static T bForceEyesToAttachment() { return ( T ) 0x29FC; }
      template<typename T = unsigned long> __forceinline static T vecEyeExitEndpoint() { return ( T ) 0x29F0; }
      template<typename T = unsigned long> __forceinline static T vehicleView_bClampEyeAngles() { return ( T ) 0x2A80; }
      template<typename T = unsigned long> __forceinline static T vehicleView_flPitchCurveZero() { return ( T ) 0x2A84; }
      template<typename T = unsigned long> __forceinline static T vehicleView_flPitchCurveLinear() { return ( T ) 0x2A88; }
      template<typename T = unsigned long> __forceinline static T vehicleView_flRollCurveZero() { return ( T ) 0x2A8C; }
      template<typename T = unsigned long> __forceinline static T vehicleView_flRollCurveLinear() { return ( T ) 0x2A90; }
      template<typename T = unsigned long> __forceinline static T vehicleView_flFOV() { return ( T ) 0x2A94; }
      template<typename T = unsigned long> __forceinline static T vehicleView_flYawMin() { return ( T ) 0x2A98; }
      template<typename T = unsigned long> __forceinline static T vehicleView_flYawMax() { return ( T ) 0x2A9C; }
      template<typename T = unsigned long> __forceinline static T vehicleView_flPitchMin() { return ( T ) 0x2AA0; }
      template<typename T = unsigned long> __forceinline static T vehicleView_flPitchMax() { return ( T ) 0x2AA4; }
   };
};
