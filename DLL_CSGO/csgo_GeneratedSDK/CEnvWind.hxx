    ...
    public:
      ...
      class DT_EnvWindShared
      {
       public:
         ...
         template<typename T = unsigned long> __forceinline static T iMinWind() { return ( T ) 0x9E4; }
         template<typename T = unsigned long> __forceinline static T iMaxWind() { return ( T ) 0x9E8; }
         template<typename T = unsigned long> __forceinline static T iMinGust() { return ( T ) 0x9F0; }
         template<typename T = unsigned long> __forceinline static T iMaxGust() { return ( T ) 0x9F4; }
         template<typename T = unsigned long> __forceinline static T flMinGustDelay() { return ( T ) 0x9F8; }
         template<typename T = unsigned long> __forceinline static T flMaxGustDelay() { return ( T ) 0x9FC; }
         template<typename T = unsigned long> __forceinline static T iGustDirChange() { return ( T ) 0xA04; }
         template<typename T = unsigned long> __forceinline static T iWindSeed() { return ( T ) 0x9E0; }
         template<typename T = unsigned long> __forceinline static T iInitialWindDir() { return ( T ) 0xA44; }
         template<typename T = unsigned long> __forceinline static T flInitialWindSpeed() { return ( T ) 0xA48; }
         template<typename T = unsigned long> __forceinline static T flStartTime() { return ( T ) 0x9DC; }
         template<typename T = unsigned long> __forceinline static T flGustDuration() { return ( T ) 0xA00; }
      };
   };
};
