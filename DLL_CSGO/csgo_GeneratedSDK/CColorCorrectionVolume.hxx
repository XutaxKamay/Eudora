...
#ifndef SDK_CColorCorrectionVolume
#define SDK_CColorCorrectionVolume
#pragma once
       ...
       public:
         ...
         class DT_BaseToggle
         {
          public:
            ...
            template<typename T = unsigned long> __forceinline static T vecFinalDest() { return ( T ) 0x9EC; }
            template<typename T = unsigned long> __forceinline static T movementType() { return ( T ) 0x9F8; }
            template<typename T = unsigned long> __forceinline static T flMoveTargetTime() { return ( T ) 0x9FC; }
         };

         template<typename T = unsigned long> __forceinline static T bClientSidePredicted() { return ( T ) 0xA08; }
         template<typename T = unsigned long> __forceinline static T spawnflags() { return ( T ) 0x2C4; }
      };

      template<typename T = unsigned long> __forceinline static T bEnabled() { return ( T ) 0xA20; }
      template<typename T = unsigned long> __forceinline static T MaxWeight() { return ( T ) 0xA24; }
      template<typename T = unsigned long> __forceinline static T FadeDuration() { return ( T ) 0xA28; }
      template<typename T = unsigned long> __forceinline static T Weight() { return ( T ) 0xA2C; }
      template<typename T = unsigned long> __forceinline static T lookupFilename() { return ( T ) 0xA30; }
   };
};
