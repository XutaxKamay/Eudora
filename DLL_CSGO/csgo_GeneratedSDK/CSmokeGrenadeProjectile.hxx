...
#ifndef SDK_CSmokeGrenadeProjectile
#define SDK_CSmokeGrenadeProjectile
#pragma once
...
class CSmokeGrenadeProjectile
{
 public:
   ...
   class DT_SmokeGrenadeProjectile
   {
    public:

      class DT_BaseCSGrenadeProjectile
      {
       public:
         ...
         class DT_BaseGrenade
         {
          public:
            ...
            template<typename T = unsigned long> __forceinline static T flDamage() { return ( T ) 0x2988; }
            template<typename T = unsigned long> __forceinline static T DmgRadius() { return ( T ) 0x2974; }
            template<typename T = unsigned long> __forceinline static T bIsLive() { return ( T ) 0x2971; }
            template<typename T = unsigned long> __forceinline static T hThrower() { return ( T ) 0x2990; }
            template<typename T = unsigned long> __forceinline static T vecVelocity() { return ( T ) 0x110; }
            template<typename T = unsigned long> __forceinline static T fFlags() { return ( T ) 0x100; }
         };

         template<typename T = unsigned long> __forceinline static T vInitialVelocity() { return ( T ) 0x29A0; }
         template<typename T = unsigned long> __forceinline static T nBounces() { return ( T ) 0x29AC; }
      };

      template<typename T = unsigned long> __forceinline static T bDidSmokeEffect() { return ( T ) 0x29D4; }
      template<typename T = unsigned long> __forceinline static T nSmokeEffectTickBegin() { return ( T ) 0x29D0; }
   };
};
