...
#ifndef SDK_CPointCommentaryNode
#define SDK_CPointCommentaryNode
#pragma once
 ...
 public:
   ...
   class DT_PointCommentaryNode
   {
    public:
      ...
      template<typename T = unsigned long> __forceinline static T bActive() { return ( T ) 0x2970; }
      template<typename T = unsigned long> __forceinline static T flStartTime() { return ( T ) 0x2974; }
      template<typename T = unsigned long> __forceinline static T iszCommentaryFile() { return ( T ) 0x2978; }
      template<typename T = unsigned long> __forceinline static T iszCommentaryFileNoHDR() { return ( T ) 0x2A7C; }
      template<typename T = unsigned long> __forceinline static T iszSpeakers() { return ( T ) 0x2B80; }
      template<typename T = unsigned long> __forceinline static T iNodeNumber() { return ( T ) 0x2C80; }
      template<typename T = unsigned long> __forceinline static T iNodeNumberMax() { return ( T ) 0x2C84; }
      template<typename T = unsigned long> __forceinline static T hViewPosition() { return ( T ) 0x2C8C; }
   };
};
