...
#ifndef SDK_CPropJeep
#define SDK_CPropJeep
#pragma once
    ...
    public:
      ...
      class DT_PropVehicleDriveable
      {
       public:
         ...
         template<typename T = unsigned long> __forceinline static T hPlayer() { return ( T ) 0x2974; }
         template<typename T = unsigned long> __forceinline static T nSpeed() { return ( T ) 0x2978; }
         template<typename T = unsigned long> __forceinline static T nRPM() { return ( T ) 0x297C; }
         template<typename T = unsigned long> __forceinline static T flThrottle() { return ( T ) 0x2980; }
         template<typename T = unsigned long> __forceinline static T nBoostTimeLeft() { return ( T ) 0x2984; }
         template<typename T = unsigned long> __forceinline static T nHasBoost() { return ( T ) 0x2988; }
         template<typename T = unsigned long> __forceinline static T nScannerDisabledWeapons() { return ( T ) 0x298C; }
         template<typename T = unsigned long> __forceinline static T nScannerDisabledVehicle() { return ( T ) 0x2990; }
         template<typename T = unsigned long> __forceinline static T bEnterAnimOn() { return ( T ) 0x29B0; }
         template<typename T = unsigned long> __forceinline static T bExitAnimOn() { return ( T ) 0x29B1; }
         template<typename T = unsigned long> __forceinline static T bUnableToFire() { return ( T ) 0x29FD; }
         template<typename T = unsigned long> __forceinline static T vecEyeExitEndpoint() { return ( T ) 0x29F0; }
         template<typename T = unsigned long> __forceinline static T bHasGun() { return ( T ) 0x29FC; }
         template<typename T = unsigned long> __forceinline static T vecGunCrosshair() { return ( T ) 0x29B8; }
      };

      template<typename T = unsigned long> __forceinline static T bHeadlightIsOn() { return ( T ) 0x2ABC; }
   };
};
