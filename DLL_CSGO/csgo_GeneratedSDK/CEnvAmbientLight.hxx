...
#ifndef SDK_CEnvAmbientLight
#define SDK_CEnvAmbientLight
#pragma once
...
class CEnvAmbientLight
{
 public:
   ...
   class DT_EnvAmbientLight
   {
    public:

      class DT_SpatialEntity
      {
       public:
         ...
         template<typename T = unsigned long> __forceinline static T flCurWeight() { return ( T ) 0x9EC; }
         template<typename T = unsigned long> __forceinline static T bEnabled() { return ( T ) 0xAF4; }
      };

      template<typename T = unsigned long> __forceinline static T vecColor() { return ( T ) 0xB00; }
   };
};
