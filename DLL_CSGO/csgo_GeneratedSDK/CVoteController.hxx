...
#ifndef SDK_CVoteController
#define SDK_CVoteController
#pragma once
 ...
 public:
   ...
   class DT_VoteController
   {
    public:
      ...
      template<typename T = unsigned long> __forceinline static T iActiveIssueIndex() { return ( T ) 0x9E4; }
      template<typename T = unsigned long> __forceinline static T iOnlyTeamToVote() { return ( T ) 0x9E8; }

      class m_nVoteOptionCount
      {
       public:

         template<typename T = unsigned long> __forceinline static T _000() { return ( T ) 0x9EC; }
         template<typename T = unsigned long> __forceinline static T _001() { return ( T ) 0x9F0; }
         template<typename T = unsigned long> __forceinline static T _002() { return ( T ) 0x9F4; }
         template<typename T = unsigned long> __forceinline static T _003() { return ( T ) 0x9F8; }
         template<typename T = unsigned long> __forceinline static T _004() { return ( T ) 0x9FC; }
      };

      template<typename T = unsigned long> __forceinline static T nPotentialVotes() { return ( T ) 0xA04; }
      template<typename T = unsigned long> __forceinline static T bIsYesNoVote() { return ( T ) 0xA0A; }
   };
};
