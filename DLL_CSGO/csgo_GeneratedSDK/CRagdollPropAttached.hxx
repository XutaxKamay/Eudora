...
#ifndef SDK_CRagdollPropAttached
#define SDK_CRagdollPropAttached
#pragma once
    ...
    public:
      ...
      class DT_Ragdoll
      {
       public:
         ...
         template<typename T = unsigned long> __forceinline static T ragAngles_0() { return ( T ) 0x2A90; }
         template<typename T = unsigned long> __forceinline static T ragPos_0() { return ( T ) 0x2970; }
         template<typename T = unsigned long> __forceinline static T hUnragdoll() { return ( T ) 0x2C6C; }
         template<typename T = unsigned long> __forceinline static T flBlendWeight() { return ( T ) 0x2C70; }
         template<typename T = unsigned long> __forceinline static T nOverlaySequence() { return ( T ) 0x2C78; }
      };

      template<typename T = unsigned long> __forceinline static T boneIndexAttached() { return ( T ) 0x2CA8; }
      template<typename T = unsigned long> __forceinline static T ragdollAttachedObjectIndex() { return ( T ) 0x2CA4; }
      template<typename T = unsigned long> __forceinline static T attachmentPointBoneSpace() { return ( T ) 0x2C80; }
      template<typename T = unsigned long> __forceinline static T attachmentPointRagdollSpace() { return ( T ) 0x2C98; }
   };
};
