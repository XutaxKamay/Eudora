...
void RenderWireframeSphere( const Vector &vCenter , float flRadius , unsigned short nTheta , unsigned short nPhi , Color c , bool bZBuffer )
{
    ...
    pRenderContext->Bind( bZBuffer ? Mat_Wireframe : Mat_WireframeIgnoreZ );
    ...
    for ( i = 0; i < nPhi; ++i )
    {
        for ( j = 0; j < nTheta; ++j )
        {
            ...
            float theta = 2.0f * Pi<float> * u;
            float phi = Pi<float> * v;
...
{
    IMaterial *pMaterial = bZBuffer ? Mat_VertexColor : Mat_VertexColorIgnoreZ;
...
static void GenerateBoxVertices( const Vector &vOrigin , const Angle& angles , const Vector &vMins , const Vector &vMaxs , Vector pVerts[ 8 ] )
{
    // Build a rotation matrix from orientation
    Matrix3x4 fRotateMatrix;
...
// Renders a solid box
//-----------------------------------------------------------------------------
void RenderBox( const Vector& vOrigin , const Angle& angles , const Vector& vMins , const Vector& vMaxs , Color c , IMaterial *pMaterial , bool bInsideOut )
{
    CMatRenderContextPtr pRenderContext( MaterialSystem.pClass );
    pRenderContext->Bind( pMaterial );
...
void RenderBox( const Vector& vOrigin , const Angle& angles , const Vector& vMins , const Vector& vMaxs , Color c , bool bZBuffer , bool bInsideOut )
{
    IMaterial *pMaterial = bZBuffer ? Mat_VertexColor : Mat_VertexColorIgnoreZ;
    Color cActual( c.r() , c.g() , c.b() , c.a() );
                               ...
                               int nXDice , int nYDice )							// Amount to tessellate the
{
    ...
    flRightX = FLerp( -1.0f , 1.0f , 0.0f , ( float ) vw , flRightX );
    flLeftX = FLerp( -1.0f , 1.0f , 0.0f , ( float ) vw , flLeftX );
    flTopY = FLerp( 1.0f , -1.0f , 0.0f , ( float ) vh , flTopY );
    flBottomY = FLerp( 1.0f , -1.0f , 0.0f , ( float ) vh , flBottomY );
...
void RenderQuad( float x , float y , float w , float h ,
                 float z , float s0 , float t0 , float s1 , float t1 , const Color& clr , bool IgnoreZ )
{
    CMatRenderContextPtr pRenderContext( MaterialSystem.pClass );
    IMesh *pMesh = pRenderContext->GetDynamicMesh( true , NULL , NULL , IgnoreZ ? Mat_VertexColor : Mat_VertexColorIgnoreZ );
    ...
    meshBuilder.Color4ub( clr.r() , clr.g() , clr.b() , clr.a() );
    meshBuilder.TexCoord2f( 0 , s1 , t0 );
    meshBuilder.Position3f( x , y + w , z );
    meshBuilder.AdvanceVertex();

    meshBuilder.Color4ub( clr.r() , clr.g() , clr.b() , clr.a() );
    meshBuilder.TexCoord2f( 0 , s1 , t1 );
    meshBuilder.Position3f( x , y + w , z + h );
    meshBuilder.AdvanceVertex();

    meshBuilder.Color4ub( clr.r() , clr.g() , clr.b() , clr.a() );
    meshBuilder.TexCoord2f( 0 , s0 , t1 );
    meshBuilder.Position3f( x , y , z + h );
    meshBuilder.AdvanceVertex();
